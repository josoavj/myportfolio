...
├── web-v1.3.0/      # Dernière version (web build)
├── current/         # Symlink → web-v1.3.0 (servie par Netlify)
...
```bash
cp -r build/web releases/web-v1.4.0
```

### Étape 3 : Mettre à jour le symlink
```bash
cd releases
ln -sf web-v1.4.0 current
cd ..
```
...
```bash
git add releases/
git commit -m "release: v1.4.0 - web build"
